...
State = set_initial_state(N, nT, partition_order);
...
function State = set_initial_state(N, num_T, options)
%function State = set_initial_state(N, num_T, params)

switch options.type_of_state
    case 'partition'
        N = params.number_of_elements;
        num_T = params.number_of_temperatures;
        K = params.number_of_states;
    ...
    case 'subset_lt1'
        ...
        State = zeros(N, num_T);
        for i_T = 1:num_T
            while sum(State(:,i_T)) < 2
                State(:,i_T) = randi(2, N, 1)-1;
            end
        end
end
